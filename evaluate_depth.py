...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.quantization:
            ...
            if opt.calibrate:
                cali_filenames = readlines(os.path.join(splits_dir, opt.split, "train_files.txt"))
                cali_dataset = datasets.KITTIRAWDataset(opt.data_path, cali_filenames,
                                                        192, 640,
                                                        [0], 4, is_train=False)
                cali_dataloader = DataLoader(cali_dataset, 16, shuffle=False, num_workers=12,
                                             pin_memory=True, drop_last=False)
                encoder.eval()
                with torch.no_grad():
                    print("[INFO] Calibrating...")
                    for data in cali_dataloader:
                        input_color = data[("color", 0, 0)]
                        input_color = input_color.to(device)
                        encoder(input_color)
                torch.quantization.convert(encoder, inplace=True)
                print(encoder)
                torch.save(encoder.state_dict(), os.path.join(opt.load_weights_folder, 'qmodel.pth'))
            else:
                torch.quantization.convert(encoder, inplace=True)
                load_quant = torch.load(os.path.join(opt.load_weights_folder, 'qmodel.pth'))
                encoder.load_state_dict(load_quant)

        encoder.to(device)
        encoder.eval()

        depth_decoder = torch.load(decoder_path)
        ...
        if opt.obstacle_detection:
            ...
            for m in mask_list:
                ...
                if np.array(pred_temp) != [] and np.array(gt_temp) != []:
                    ...
                    pred_temp = np.max(pred_temp)
                    gt_temp = np.max(gt_temp)
